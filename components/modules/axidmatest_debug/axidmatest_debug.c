...
static int dmatest_slave_func(void *data)
{
    ...
    pr_err("Using alignment value %d", align);
		...
		for (i = 0; i < src_cnt; i++) {
			...
			pr_err("Mapping tx descriptor %d to %p(%d)\n",i, buf, len);

			dma_srcs[i] = dma_map_single(tx_dev->dev, buf, len,
							DMA_MEM_TO_DEV);
      if (dma_mapping_error(tx_dev->dev, dma_srcs[i])) {
			  pr_err("Mapping tx descriptor has failed\n");
        break;
      }
		}

    //TODO: We shoud free all the previous mappings
    if (i != src_cnt)
      break;
    ...
    // We need to issue the RX Channel first! Otherwise we discard the messages!
					...
					struct dmatest_chan *rx_dtc)
{
	...
	if (!thread) {
		pr_err("dmatest: No memory for slave thread %s-%s\n",
	...
	if (IS_ERR(thread->task)) {
		pr_err("dmatest: Failed to run thread %s-%s\n",
				dma_chan_name(tx_chan), dma_chan_name(rx_chan));
		kfree(thread);
	}
					...
					struct dma_chan *rx_chan)
{
	...
	if (!tx_dtc) {
		pr_err("dmatest: No memory for tx %s\n",
	...
	if (!rx_dtc) {
		pr_err("dmatest: No memory for rx %s\n",
